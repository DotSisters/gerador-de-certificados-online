...
namespace GeradorDeCertificados.WebApi.Compartilhado.Logging;
...
public static class LoggingExtensions
{
    ...
    )
    {
        ...
        string caminhoDiretorio = Path.Combine(caminhoAppData, "GeradorDeCertificados");
        ...
        if (newRelicOptions.Enabled)
        {
            if (string.IsNullOrWhiteSpace(newRelicOptions.LicenseKey))
            {
                throw new InvalidOperationException(
                    "A chave de licença do New Relic não foi configurada. Configure NewRelic:LicenseKey."
                );
            }
